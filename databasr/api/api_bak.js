...
router.post('/getSubmission', verify, async (req, res) => {
    try{
        ...
        filesystem.readdir(submissionDirectory, (err, files) => {
            ...
            files.forEach((file) => {
                if(file === '.submiton'){
                    filesystem.readFile(path.join(submissionDirectory, '.submiton'), 'utf-8', (err, data) => {
                        if(err){
                        ...
                        //TODO- get date
                        submittedOn = Date(data);
                    })
                }

                else if(file === '.graded'){
                    filesystem.readFile(path.join(submissionDirectory, '.graded'), 'utf-8', (err, data) => {
                        if(err){
